...
/// ## Null handling
///
/// - **Regular fields**: When null, delegates to serde (`Ok(None)` for `Option<T>`, error otherwise)
/// - **Array fields**: Returns `Option<Vec<_>>` - `None` if null, `Some(vec)` otherwise.
///   Users must declare array fields as `Option<Vec<T>>`.
///
...
fn generate_from_segments(full_path: &str, segments: &[PathSegment<'_>]) -> TokenStream2 {
    ...
    if segment.is_array {
        ...
        // Regular field: navigate and handle null
        quote! {
            {
